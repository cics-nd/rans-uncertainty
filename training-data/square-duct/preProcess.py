...
if __name__ == '__main__':
    ...
    # NOTE: for square duct we average in x direction!
    # Get unique y & z coords
    cell_n = cell_centers.numpy()
    cell_yz = np.unique(cell_n[:,1:], axis=0)
    saveTensor(th.from_numpy(cell_yz), 'cellCenters', rans_time, rans_dir)
    
    # Now get averaging indexes (where y & z are the same)
    avg_index = []
    for i in range(cell_yz.shape[0]):
        if(i%100 == 0):
            print('Finding average indexes {}/{}'.format(i, len(cell_yz)))
        avg_index.append(np.where(np.all(cell_n[:,1:] == cell_yz[i], axis=1))[0])
